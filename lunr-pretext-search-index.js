...
},
{
  ...
  "body": " Changes Needed  Needed changes are now tracked on Github  "
},
{
  "id": "front-colophon",
  "level": "1",
...
},
{
  ...
  "type": "Figure",
  "number": "15.13.1",
  "title": "Deep Space Network Satellite Coverage [88]",
...
{
  "id": "sec-spacecraft-attitude-control-schemes",
  "level": "1",
  "url": "sec-spacecraft-attitude-control-schemes.html",
  "type": "Section",
  "number": "17.1",
  "title": "Spacecraft Attitude Control Schemes",
  "body": " Spacecraft Attitude Control Schemes   Many control schemes are needed to orient a satellite and all depend on the application. In LEO magnetorquers can be used to detumble a satellite while thrusters must be used in deep space. In addition reaction wheels can be used to detumble a satellite anywhere in space provided the angular momentum in the satellite does not saturate the reaction wheels. Sections that follow detail the control schemes typically utilized on small sats. A section on PID control for aircraft is also in this section.    B-dot Controller  In LEO, the standard B-dot controller reported in many sources ( , , , ) can be used to de-tumble a satellite. The standard B-dot controller requires the magnetorquers to follow the control law shown below   where is the control gain. Using equation ( ) it is possible to write the current in component form again using the identity that    This equation can then be substituted into equation ( ) to produce the total torque on the satellite assuming that the magnetorquers can provide the necessary current commanded by equation ( ).   The goal of the controller here is to drive . The literature will show that this is not completely achieved . There are multiple explanations for this. For starters, equation ( ) assumes that the magnetic moment is not co-linear with the magnetic field of the Earth. If it is, the result is zero torque applied to the satellite. Furthermore, equation ( ) results in zero current if the angular velocity vector of the satellite is co-linear with the magnetic field. Thus, if the magnetic field vector, angular velocity vector or the magnetic moment vector are co-linear, the torque applied to the satellite will be zero. If a new operator is defined such that   it is easy to see that the torque applied to a satellite is then simply the angular velocity vector multiplied by this transition matrix. If this transition matrix is put into row-reduced-echelon form it is easy to see that the determinant of this matrix is equal to zero ( ).   A zero determinant means that there exists a vector that will result in zero torque for a given value of the magnetic field. This is typically avoided since the magnetic field of the Earth is time and spatially varying which results in a transition matrix that changes over time due to orientation changes in the satellite as well as changes in the satellite's orbit. However, for low inclination orbits, it's possible for the magnetic field to stay relatively constant with . If the satellite is tumbling about the yaw axis such that , the yaw torque on the satellite ( ) will be zero. Using this simple controller, there is no way to remove the remaining angular velocity from the satellite unless reaction wheels are used.    Reaction Wheel Control  Assuming each reaction is aligned with a principal axis of inertia the control scheme is extremely simple. When the wheels are not aligned the derivation will proceed similar to the reaction control thruster section. The derivation here will just be for the aligned case. In this analysis it is assumed that a torque can be applied to the reaction wheel and thus the angular velocity of the reaction wheel can be directly controlled. Assuming this a simple PD control law can be used to orient the satellite at any desired orientation using Euler angles for this control law since the satellites are aligned with the principal axes of rotation .   In the equation above denotes either roll , pitch or yaw depending on which reaction wheel is being used. The Euler angles in this case would be obtained by converting the quaternions to Euler angles as defined in .  Often times however your reaction wheels are not pointed on the principal axis of inertia. In this case a Least Squares Regression model is needed. In this case the equation above is used to compute the desired torque to be placed on the satellite such that   This equation is then equated to the equation for torque placed on the satellite where the angular accelerations are placed into a vector.   Since is a matrix its impossible to simply invert the matrix and solve for the vector of angular accelerations . In this case there are an infinite number of solutions. As such a minimization routine is required where the solution found also happens to be the lowest amount of angular acceleration. In this case, Lagrange's method was used to find the vector of angular accelerations .     Reaction Control Thrusters  The control law for the thrusters is a bit complex if the location of thrusters is not know apriori. If the location is known then simple PID control laws can be generated by applying pure couples to the correct thrusters that activate the correct axes. If the location is not known then the following derivation will suffice. There are thrusters and only 3 degrees of freedom that need to be controlled; thus, the system is an overactuated system. Using equation , the equation can be written in matrix form as given by the equation below where is replaced by . The equation for is generated using a similar PD control law as the reaction wheels.   Since is a matrix its impossible to simply invert the matrix and solve for the vector of pulses . In this case there are an infinite number of solutions. As such a minimization routine is required where the solution found also happens to be the least amount of pulses. In this case, Lagrange's method was used to find the vector of pulses .   Note that a similar equation can be derived for . The solution to the equation above results in values of that are bigger than 1 and sometimes negative. If a value in this vector is bigger than 0 the value is set to 1 and if the value is negative the value is set to 0. Thus, the solution does not yield an exact solution but it does allow for flexibility in the number of thrusters and their respective orientations. Sizing of the thrusters depends on many independent variables including the thrust and the . Using the the exit velocity of the thruster can be obtained by using the equation below   where is the gravitational acceleration of the Earth at sea-level. Then the mass flow rate of the thruster can be obtained using the equation below.   Using this mass flow rate total propellant mass required can be computed assuming a certain duty cycle.    Cross Products of Inertia Control  An interesting form of control is to take advantage of momentum dumping. Looking at the equation for angular acceleration again ( ) this equation can be simplified for certain cases. For example, if the roll rate of the satellite is set to be non-zero while the pitch rate and yaw rates are set to zero it is easy to see that if the inertia is diagonal the derivative of angular velocity is zero. However, if the cross products of inertia are given by the matrix below   the derivative of angular velocity becomes   again assuming the roll rate is non zero and the pitch rate is zero. This result shows that momentum can be transferred to different axes provided the cross products of inertia are non-zero.   "
},
{
  "id": "sec-inner-outer-loop-control",
  "level": "1",
  "url": "sec-inner-outer-loop-control.html",
  "type": "Section",
  "number": "17.2",
  "title": "Inner and Outer Loop Control of an Aircraft",
  "body": " Inner and Outer Loop Control of an Aircraft   Controlling an aircraft typically requires an inner loop-outer loop control scheme. The inner loop is typically used to control the attitude of the aircraft while the outer loop is used to control the position of the aircraft. The inner loop is typically a PID controller that controls the roll, pitch and yaw of the aircraft while the outer loop is also typically a PID controller that controls the position of the aircraft in 3D space. The inner loop is generally faster than the outer loop and thus can be used to stabilize the aircraft while the outer loop can be used to control the position of the aircraft.    Inner Loop Control  For a conventional PID controller of an aircraft, the rudder, elevator and aileron commands are set to   The Euler angle commands and are set using the following relationships:   The control scheme defined above is a conventional inner loop-outer loop control of a fixed wing aircraft using a PID tracking controller.    Velocity Control    Altitude Control    Heading Angle Control of a Car    Heading Angle Control of an Airplane    Waypoint Control of a Car    Waypoint Control of an Airplane   "
},
{
...
},
{
  ...
  "type": "Section",
  "number": "19.1",
  "title": "Systems Engineering Life Cycle",
  "body": " Systems Engineering Life Cycle  The systems engineering life cycle is a more detailed version of the DBT framework. The systems engineering life cycle is used in many industries including aerospace, automotive, and software development. At the undergraduate level the following steps are used for a standard 16 week curriculum.     Week 1:  Title Page and Group Development - Students are given an explanation of the project scope, rubric are asked to split into teams to create a title page and team name. In aircraft design the project is to design, build and test a radio controlled aircraft. In spacecraft design the project is to design, build and test a model rocket. In Instrumentation and Experimental Methods the project is to design, build and test a data acquisition system with at least 3 electrical components not counting the CPU, flash memory, or power supply. This also includes mechanical subsystems so that students practice both hardware and software integration. The teams are typically 1-3 students each depending on class size.     Week 2:  Concept of Operations (ConOps) - Students are given an explanation of the ConOps and asked to create a ConOps for their project as well as include a list of requirements which is based on the project scope given by the instructor as well as any additional requirements the students feel are necessary.     Week 3:  Conceptual Design - Students are asked to create a hand sketch of their project as well as a brief list of materials they will need to complete the project. This also includes a list of subsystems and components that will be needed to complete the project which requires the students to create a Block Definition Diagram (BDD) as well as an Internal Block Diagram (IBD).     Week 4:  Preliminary Design - For the preliminary design phase the task is different depending on the course. In Aircraft Design the students are asked to perform a feasibility analysis and look at overall wingspan, chord and an initial weight estimate. In spacecraft design the students need to look at creating rockets in OpenRockt and perform a preliminary analysis of the rocket's stability. In Instrumentation and Experimental Methods the students need to create more detailed BDDs and IBDs for the mechanical, electrical and software subsystems as well as create an initial interface diagram (ID) for the project.     Weeks 5-7:  Detailed Design - The detailed design phase is broken into 3 separate weeks. For aircraft design the students are asked to design the aerodynamic surfaces (wing and tail), the airframe and finally the propulsion system. For spacecraft design the students need to design the structure, propulsion system and perform a detailed trajectory analyis of their rocket. For Instrumentation and Experimental Methods the students need to design the mechanical, electrical and software subsystems creating schematics, wiring diagrams and a list of all software needed to run their system. For every course the students also need to create a detailed bill of materials with prices, weights and links. At this point the system is ready to be built.     Week 8:  Build Plan, Purchase materials + Break - In the Fall semester, Fall break typically lands on Week 8 and in the spring Week 8 is typically spring break. As such Students are asked to create a build plan which is not something to be turned in but just something helpful to keep students on track as well as purchase all parts necessary while they are on holiday. If parts are purchased the Friday before break that typically gives the retail stores over 5 business days or 9 calendar days to ship the parts to the students before they return to class. This way students can immediately start building and testing during Week 9 when they return.     Weeks 9-11:  Build and Subsystem Testing - The build process includes building everything component by component and testing each subsystem as it is built. This is important to ensure that each subsystem works properly independently before integrating everything together. For aircraft design the students will first build the airframe, then the propulsion system and finally the payload which is typically just a datalogger that measures acceleration and GPS location. For spacecraft design the students will first build the structure, then the propulsion system and finally the payload which for spacecraft design is a datalogger that measures acceleration and pressure. For Instrumentation and Experimental Methods the students will test each electrical component one by one and test the software for that specific component. The mechanical system is something to be built and integrated in Week 12.     Weeks 12-13:  System Integration and Testing - The integration process includes integrating all subsystems together and testing the entire system as a whole. For aircraft design the students will integrate the airframe, propulsion system and payload and perform a ground test to ensure everything works properly. For spacecraft design the students will integrate the structure and payload and perform a series of tests to ensure everything works properly. This includes a shock cord test, a drop test, a datalogger test, a motor integration test and a nose cone test. For Instrumentation and Experimental Methods the students will integrate all electrical and software subsystems and perform a test without the mechanical subsystem in the way.     Weeks 14-16:  Final Testing and Validation - The final testing and validation phase includes performing the final test of the entire system. For aircraft design the students will perform a flight test of their aircraft and recover the data from the datalogger. For spacecraft design the students will perform a launch of their rocket and recover the data from the datalogger. For Instrumentation and Experimental Methods the students will integrate the mechanical subsystem and perform a final test of the entire system. Note that 3 weeks are alloted for this phase to account for weather delays, part delays and any other unforeseen issues that may arise. Many times especially in aircraft design the system fails sometimes catastrophically as in a crash and the students need time to rebuild and retest before the final presentation.     Week 17:  Final Report and Presentation - Although a semester is exactly 16 weeks, the final report and presentation is typically due the first week of finals which ends up being Week 17. The final report includes a summary of the entire project including all diagrams, analyses, test results and conclusions. The final presentation is a 10-15 minute presentation to the class and any invited guests which summarizes the entire project. Note that the final report and presentation is typically worth 30-40% of the overall grade so it is important that students take this seriously and put in their best effort. Furthermore, there are no exams or quizzes in these project based courses so the final report and presentation is the only way to demonstrate what the students have learned throughout the semester. This gives the students ownership of the project and motivates them to do their best work.    "
},
{
  "id": "sec-bdds",
  "level": "1",
  ...
  "type": "Section",
  "number": "19.2",
  "title": "Block Definition Diagrams (BDDs)",
  "body": " Block Definition Diagrams (BDDs)     "
},
{
  "id": "sec-ibds",
  "level": "1",
  ...
  "type": "Section",
  "number": "19.3",
  "title": "Internal Block Diagrams (IBDs)",
  "body": " Internal Block Diagrams (IBDs)     "
},
{
  "id": "sec-interface-diagrams",
  "level": "1",
  "url": "sec-interface-diagrams.html",
  "type": "Section",
  "number": "19.4",
  "title": "Interface Diagrams",
  "body": " Interface Diagrams  Interface Diagrams (IDs) are important for many reasons. The biggest reason that these diagrams are created and maintained is to create visibility within each subsystem team in a systems engineering project. It is crucial that each member working on a project has a general or even an expert understanding of each subsystem. During the design process, it is important that each function of each subsystem’s component is documented so that the current design selection is recorded and so each of the member’s is familiar with each subsystem and all of its functionalities. The following image is the ID for the attitude control and maneuver electronics subsystem on the Gemini Spacecraft.   ACME Interface Diagram from the Gemini Spacecraft    This is an example of an interface diagram for a small subsystem of a large systems engineering project. The spacecraft requires several subsystems to work together in the design and fabrication process. Each of these subsystems has their own ID as well. This is a good example to introduce you to what these diagrams are. The left side of the image is the digital computer which has an accumulator which flows to the ladder logic block. From the ladder logic flows several functions into both the attitude control and maneuver electronics and attitude display blocks. This ID is simple since there are only four components with many functions, however, many complicated systems can have extremely complex diagrams. The Figure below is an example of an ID for an entire spacecraft with all of the subsystems included in the diagram.   New Horizons Interface Diagram    "
},
{
  "id": "fig-acme-id",
  "level": "2",
  "url": "sec-interface-diagrams.html#fig-acme-id",
  "type": "Figure",
  "number": "19.4.1",
  "title": "",
  "body": " ACME Interface Diagram from the Gemini Spacecraft   "
},
{
  "id": "fig-newhorizons-id",
  "level": "2",
  "url": "sec-interface-diagrams.html#fig-newhorizons-id",
  "type": "Figure",
  "number": "19.4.2",
  "title": "",
  "body": " New Horizons Interface Diagram   "
},
{
  "id": "sec-activity-diagrams",
  "level": "1",
  "url": "sec-activity-diagrams.html",
  "type": "Section",
  "number": "19.5",
  "title": "Activity Diagrams",
  "body": " Activity Diagrams  Activity diagrams are similar to a flowchart. It is a tool for representing the sequence of Actions that describe the behavior of a Block or other structural element. The sequence of execution is defined using Control Flows. The Actions in an activity diagram can contain Input and Output Pins which act as buffers for items that flow from one action to another. Anything that can be produced, consumed, or conveyed by the system is considered to be an item. Physical materials, energy, power, data, and information are examples of items   Activity diagrams are useful for engineering modeling. It conveys high-level functions and operations to the user. The main purpose of an activity diagram is to draw the activity or action flow of a system. Next, it is used to describe the sequence from one activity to another. Finally, an activity diagram describes the parallel, branched, and concurrent flow of the system .  Activity diagrams are important to the design process because they maintain coherence throughout the project. If a system is composed of multiple, intertwining subsystems, a person can simply follow the activity diagrams to understand how each component and subsystem interacts with one another. Activity diagrams also describe the data flow of a system and when or where that data is created, converted or used. Overall, activity diagrams are the roadmap of a system and describe the intricacies of how it operates.  As stated previously, activity diagrams denote how components and subsystems interact with each other. This is accomplished through the use of swimlanes which keep separate the individual activities and object flows of a component or subsystem. Swimlanes are the boxes which contain activities, however control and object flows can traverse swimlanes. Activities which have inputs and outputs will also have a pin attached to them. This small rectangular box denotes when matter, energy or data flow is created or destroyed.  The main purpose of activity diagrams is to show the control flow of the system. This control flow refers to the execution path of an activity. Control flow is indicated by control nodes for which there are seven different kinds. Each control node is described in the table below.   Control Node Descriptions    Control Node  Description  Appearance    Initial Node  The beginning of an activity sequence  Black circle    Activity Final Node  The end of an activity sequence  Black circle within a circle    Flow Final Node  The end of one branch of an activity sequence, but not the entire activity diagram.  Circle with an 'X' in it    Decision Node  The splitting point of an activity flow based on the outcome of a boolean operator such as 'isCommandValid ==True'. One flow in comes in, while two flows come out as branching paths.  Diamond    Merge Node  The merging point of two activity paths. Two flows come in, while one flow comes out.  Diamond    Fork Node  The distribution of an activity flow into multiple pathways. When one object or control token comes in, it is duplicated into multiple paths.  Black bar    Join Node  The joining point of multiple synchronized concurrent flows. Two concurrent flows come in, while one flow comes out.  Black bar     "
},
{
  "id": "table-control-nodes",
  "level": "2",
  "url": "sec-activity-diagrams.html#table-control-nodes",
  "type": "Table",
  "number": "19.5.1",
  "title": "Control Node Descriptions",
  "body": " Control Node Descriptions    Control Node  Description  Appearance    Initial Node  The beginning of an activity sequence  Black circle    Activity Final Node  The end of an activity sequence  Black circle within a circle    Flow Final Node  The end of one branch of an activity sequence, but not the entire activity diagram.  Circle with an 'X' in it    Decision Node  The splitting point of an activity flow based on the outcome of a boolean operator such as 'isCommandValid ==True'. One flow in comes in, while two flows come out as branching paths.  Diamond    Merge Node  The merging point of two activity paths. Two flows come in, while one flow comes out.  Diamond    Fork Node  The distribution of an activity flow into multiple pathways. When one object or control token comes in, it is duplicated into multiple paths.  Black bar    Join Node  The joining point of multiple synchronized concurrent flows. Two concurrent flows come in, while one flow comes out.  Black bar    "
},
{
  "id": "sec-trajectory-analysis",
  "level": "1",
  "url": "sec-trajectory-analysis.html",
  "type": "Section",
  "number": "20.1",
  "title": "Trajectory Analysis",
  "body": " Trajectory Analysis  The trajectory of a spacecraft is crucial to the GNC subsystem. The type of orbit that the spacecraft is designed for plays a large role in determining the components chosen for the attitude control, attitude estimation, and position estimation.  There are five main trajectories that spacecrafts typically follow:    LEO (Low Earth Orbit) - Things that must be taken into account in this trajectory are aerodynamics, Earth albedo, and space debris.    MEO (Medium Earth Orbit) - Van Allen Belts, which are radiation belts, need to be considered during the design process.    GEO (Geosynchronous Orbit) - Most communication satellites are in geostationary orbit since the satellite appears stationary and the communication dishes can be pointed directly at them and do not need to be maneuvered to follow the satellite.    HEO (High Earth Orbit or Highly Elliptical Orbit) - In this case you might be well outside of the GPS constellation, thus, constant position must be considered by an alternate means.    Deep Space - A deep space orbit is largely outside of any protective atmosphere, therefore radiation is a dominant concern. Also, well beyond the GPS constellation.    Propulsion, magnetorquers, and reaction wheels are the three main mechanisms for controlling the attitude of the spacecraft. Reaction wheels can be used universally, however, magnetorquers are only useful within a magnetic field. Thus, within Low Earth Orbit (LEO) magnetorquers are incredibly useful, however in highly elliptical orbit (HEO) they may not be nearly as useful. As for the third option, propulsion, it is really only useful outside of a magnetic field when magnetorquers are not ideal. A middle Earth orbit (MEO) is unique in that it can use everything that is offered in the LEO and HEO but it will continually travel through the Van Allen Belts. The Van Allen Belts are pockets of radiation trapped by the Earth’s magnetic field which require additional shielding onboard the spacecraft to mitigate its effects.    In LEO, it is common to use a Global Positioning System (GPS) to estimate the position of the spacecraft. This component is most useful at lower altitudes because it is able to reach the GPS signal, however, as the altitude increases and the spacecraft moves farther away from the signal, it is no longer helpful. This is when an integration scheme may be the best possibility for position estimation. Integration schemes, such as RK-11, are tedious but are commonly used in HEO. Since HEO’s have an enormous apogee compared to perigee, up to a 35:1 ratio, the need for an alternate means to determine position is imperative  .  Much like a HEO, any deep space trajectory will more than likely take the spacecraft outside of the GPS constellation. Therefore, in order to determine position the spacecraft must use NASA’s deep space network, a numerical analysis, or another mathematical means. Communication with the spacecraft is another complex variable when in a deep space trajectory. Because of the large amount of latency between the ground and the spacecraft, the spacecraft will need to be able to autonomously operate without a constant uplink from the ground. Lastly, the spacecraft will need additional radiation shielding when in deep space to protect itself. This will further increase its mass and add further constraints to the GNC subsystem.  Attitude estimation can be determined by the use of magnetometers, however, similar to magnetorquers, they are only useful within a magnetic field. Thus, magnetometers are used when the spacecraft is set to be in LEO. There are several methods that can be used for attitude estimation, however, the only other one that greatly depends on the orbit are horizon sensors which must be used in LEO.  "
},
{
  "id": "sec-spacecraft-environment",
  "level": "1",
  "url": "sec-spacecraft-environment.html",
  "type": "Section",
  "number": "20.2",
  "title": "Spacecraft Environment",
  "body": " Spacecraft Environment  The environment of space can have an enormous impact on the spacecraft and the GNC subsystem. The environment is dependent on the trajectory of the spacecraft and at what altitude it is set to reach. The environment of space changes as the spacecraft travels farther from the earth and there can be negative repercussions depending on the components and materials chosen for the spacecraft.  There are four main environments that may affect a spacecraft:    Earth: Does not affect the design of the spacecraft but it is important to keep in mind that if components are not commercial off the shelf (COTS), they need to be built in a lab    Launch: It is crucial to research the vibration requirements before launching. Again, if it is not commercial off the shelf, a vibe test must be run to ensure the components survive.    LEO: This environment has many factors that must be considered including effects from the sun, thermal changes, Van Allen Belts which account for a large amount of radiation thus radiation shielding must be researched for the separate components.    Deep Space: There are some things such as meteorites that may need to be considered, however, there is not much that can be done. This environment should not heavily affect the design. Note that the spacecraft is going to undergo multiple types of disturbances as explained in the dynamic model section.    "
},
{
  "id": "sec-spacecraft-attitude-control",
  "level": "1",
  "url": "sec-spacecraft-attitude-control.html",
  "type": "Section",
  "number": "20.3",
  "title": "Spacecraft Attitude Control",
  "body": " Spacecraft Attitude Control   Attitude control is the means by which the orientation of the spacecraft is maintained within the orbit. Throughout the mission, the spacecraft experiences disturbance torques that are caused by a variety of sources, such as aerodynamics, gravity gradient, and solar radiation pressure discussed previously. These torques impart momentum which rotate the system away from its desired orientation. The purpose of attitude control is to reject these disturbance torques while simultaneously pointing towards a desired orientation. Possible active mechanisms for attitude control are magnetorquers, reaction wheels, thrust vector control, reaction control system, and control moment gyros.  There are several components that can be used for the attitude control of a satellite. Selecting the proper component is dependent on the orbit and its apogee\/perigee, the system's requirements, and the risks associated with the component. There are several steps involved with component selection. Typically the team will review the requirements and perform a preliminary risk analysis. This will generally help narrow down which components to use where. To determine the specific component, trade studies are conducted to compare the components from different manufacturers to determine the best one for the project at hand.    Magnetorquers  The first option for attitude control is a magnetorquer. A magnetorquer, or torque rod, is built from electromagnetic coils. These coils produce a magnetic field that interacts with another magnetic field, typically Earth’s, which in turn produces a torque upon the satellite . These components are used for changing the angular momentum of the satellite. In addition to attitude control, magnetorquers are also used for detumbling the satellite. Detumbling is a means of stabilizing the satellite after being inserted into the desired orbit . The International Geomagnetic Reference Field (IGRF) is a software that reports the magnetic field of the earth by using latitude, longitude, and altitude from a GPS. This magnetic field is then used to determine the proper magnetic field required by the magnetorquer to maneuver the satellite.   Example Magnetorquer    An example of magnetorquer effectiveness is its ability to detumble during an orbit, and how many orbits it will take to completely detumble the spacecraft. First, the magnetorquer will have a magnetic moment value associated with the physical parameters of the rod . First the average torque is computed by taking the magnetic moment and multiplying it with the average field strength and the point (from IGRF model). Then, this average torque is multiplied by total time of the orbit to determine the magnetorquer’s momentum effectiveness over one orbit. Note that the torque from the magnetorquer’s is substracted by the total disturbance torques to obtain the total effectiveness of the magnetorquers. From this, it is possible to calculate the number of orbits it will take to detumble the spacecraft on an axis.   The magnetorquer effectiveness boils down to the difference in the total moment that the magnetorquers can produce and the total disturbance momentum during the orbit. Then, the number of orbits that it will take for the spacecraft to detumble in an axis is a function of the magnetorquer effectiveness divided by the initial angular momentum in each axis.    Reaction Wheels  Reaction wheels are mechanical disks, or flywheels, that rotate the satellite during orbit. The reaction wheels take in electrical power and provide a torque to the satellite . This electrical power is provided to a DC motor which then spins the reaction wheel. Because momentum must be conserved, angular momentum must also be conserved. Thus, by Newton’s third law, when the reaction wheel rotates one direction the satellite rotates in the other. So, a minimum of one reaction wheel is placed on each axis to control the rotation in all directions. The primary role of a reaction wheel is to point the satellite without the use of any fuel.   Example Reaction Wheels    The inertia storage for reaction wheels is a parameter that can be used to seek out the best reaction wheels for the spacecraft. Once the inertia storage is computed, one can search for reaction wheels with the proper amount of inertia storage required. For example, if a reaction wheel with an inertia storage of 50 mNms is needed, one can go to Blue Canyon Technologies website to find the datasheets for their different reaction wheels. From there, it can be determined that the best fit would be the RWP050 reaction wheels .  Once an estimate of the inertia of the satellite is obtained, they can be used to compute the required inertia storage of the reaction wheels. The tip off rate for a spacecraft is the rate at which the spacecraft’s angular velocity is altered due to its deployment. The following equation shows how momentum can be calculated due to tip off.   where is typically set at 10 deg\/s and f the factor of safety is typically set to 2.    Thrust Vector Control (TVC)  When aerodynamic control surfaces are ineffective, TVC is a solution to maintain the vehicle's correct attitude for the thrust duration. It accomplishes this goal by gimballing (rotating) the thrust chamber or by redirecting the exhaust-gas flow to develop torque . However, thrust vectoring only moves the vehicle in two directions. TVC is typically used to control pitch and yaw attitude on boosters and upper stages.   TVC Diagram      Reaction Control System (RCS)  Reaction control systems are small thrusters. These thrusters alter the speed of the spacecraft's rotational or spinning motion. They are good for making quick turns and are used for getting to new orientations quickly. Reaction control thrusters are typically in \"couples\" or pairs of thrusters that together can spin the spacecraft without changing the lateral velocity. In addition, these thrusters usually work great with reaction wheels. When reaction wheels slow down, they produce a force. The RCS will oppose this force which allows the spacecraft to remain in the intended orientation .   Example RCS Thruster      Control Moment Gyro  A CMG is a spinning rotor at a constant speed [25]. Similar to a reaction wheel, a CMG also has a spinning fly-wheel controlled by a brushless motor. However, the spin axis of a CMG can rotate with the help of a second motor placed on a gimbal axis. If the angular momentum can only rotate in a fixed plane, it is known as a single gimbal control moment gyros (SGCMG). GMCS are usually more efficient and produce higher torque than reaction wheels as the size of the unit increases .   Example CMG     "
},
{
  "id": "fig-magnetorquer",
  "level": "2",
  "url": "sec-spacecraft-attitude-control.html#fig-magnetorquer",
  "type": "Figure",
  "number": "20.3.1",
  "title": "",
  "body": " Example Magnetorquer   "
},
{
  "id": "fig-reaction-wheels",
  "level": "2",
  "url": "sec-spacecraft-attitude-control.html#fig-reaction-wheels",
  "type": "Figure",
  "number": "20.3.2",
  "title": "",
  "body": " Example Reaction Wheels   "
},
{
  "id": "fig-tvc",
  "level": "2",
  "url": "sec-spacecraft-attitude-control.html#fig-tvc",
  "type": "Figure",
  "number": "20.3.3",
  "title": "",
  "body": " TVC Diagram   "
},
{
  "id": "fig-rcs",
  "level": "2",
  "url": "sec-spacecraft-attitude-control.html#fig-rcs",
  "type": "Figure",
  "number": "20.3.4",
  "title": "",
  "body": " Example RCS Thruster   "
},
{
  "id": "fig-cmg",
  "level": "2",
  "url": "sec-spacecraft-attitude-control.html#fig-cmg",
  "type": "Figure",
  "number": "20.3.5",
  "title": "",
  "body": " Example CMG   "
...
{
  "id": "sec-spacecraft-attitude-determination",
  "level": "1",
  "url": "sec-spacecraft-attitude-determination.html",
  "type": "Section",
  "number": "20.4",
  "title": "Spacecraft Attitude Determination",
  "body": " Spacecraft Attitude Determination   Attitude determination is a fundamental portion of the ADACS board and requires the vehicle to determine it's orientation with respect to an inertial frame. The sections that follow detail the sensors and fundamental attitude determination algorithms derived thus far.    Sensor Overview  There are a multitude of sensors that are typically used on board small sats. Note that most satellites use a combination of these sensors rather than using all of them on one single satellite.    Magnetometers: Only used in LEO, they measure the magnetic field in the body frame .    Rate Gyros: These sensors measure the angular velocity of the spacecraft in the body frame .    Solar Sensors: These sensors can be coarse analog sensors with an accuracy of 45 degrees or can be high precision digital sensors that have accuracy down to 1 degree. Sun senors return an azimuth and declination angle which can be then translated into a vector in the body frame .    Horizon Sensors: Horizon sensors are typically used in LEO as they find the horizon of the Earth and use that for orientation information. These sensors also return an azimuth and declination angle that can be translated into a body frame vector .    Startrackers: Startrackers utilize a large aperture digital camera to photograph a starmap within the field of view of the lens. The photographed stars are then cross referenced with a starmap database and return the full quaternion vector.    Some issues arise with all of these sensors. For example, magnetometers must be activated when magnetorquers are turned off otherwise those artificial magnetic fields will pollute the data. Rate gyros are prone to drift while solar sensors can be quite inaccurate. Startrackers also run the risk of being blinded by the Sun and\/or the Moon thus it is possible to design an attitude determination algorithm that utilizes the Sun's ephemeris data along with the Moon's ephemeris data in the event that the startracker is obscured by the Sun\/Moon.    Inertial Measurement Unit  An inertial measurement unit (IMU) is a combination of three sensors. An accelerometer, rate gyro and magnetometer. A magnetometer is a device that measures the local magnetic field in the body frame  . Note that the implies a measurement rather than the truth signal. Measurements from sensors are prone to bias, drift, scale factor, misalignment, noise and other sources of error that must be accounted for.   Example Magnetometer    A rate gyroscope, commonly referred to as a rate gyro measures the angular velocity also in the body frame .   Example IMU with Integrated Rate Gyro    Accelerometers are sensors used to measure acceleration at a point P on a rigid body . For simplicity however, it is assumed that point P on the rigid body is the center of mass point C therefore the accelerometer is measuring the acceleration of the body itself in the body frame with respect to an inertial frame .   Example Accelerometer    As mentioned before, the IMU consists of 3 sensors all returning 3 measurments. This results in 9 scalar quantities being returned from this sensor which is where the term 9DOF gets it origin. In reality DOF means Degrees of Freedom which is contrary to the standard 6DOF simulation models explained above. However, the sensor community chooses to coin the term 9DOF to highlight the 9 different scalar values returned from IMUs. It is possible to obtain a 10DOF sensor which also returns pressure or temperature data.    StarTracker  A star tracker is another critical component of attitude control. This device is a camera that points out from one face of the satellite. The camera is able to detect the stars in its field of view (FOV) and determine the location of the satellite based on this. Specifically, the star tracker images the “starscape” to identify the known planets and stars, and compares the imaged locations to the known locations using the SPICE Toolkit . NASA Jet Propulsion Laboratory (JPL) has produced the SPICE Toolkit for knowledge of the location of the planetary bodies and stars and specific times .   Example StarTracker      Sun Sensors  A sun sensor is a device that senses the sun’s direction to measure the position of the sun with respect to the sensor’s position. There are three types of sun sensors. The first one is an analog sensor whose output signal is a continuous function of the Sun angle. Next is a sun presence sensor which provides a constant output signal when it senses the sunlight. The last one is a digital sensor that produces encoded discrete output that is measured by the sun angle function.  Sun sensors work based on the entry of light into a thin slit on top of a rectangular chamber with the bottom part lined with a group of light-sensitive cells. The chamber casts an image of a thin line on the chamber bottom. The cells at the bottom of the rectangular chamber measure the distance of the image and the refraction angle by using the chamber height. The cells convert the incoming photons into electrons and develop voltages which are converted into digital signals. the direction of the sun can be computed when the sensors are perpendicular to each other .   Example Sun Sensor      Horizon Sensor  The horizon sensor is a sensor used by spacecraft in LEO to determine the location of the Earth’s horizon. There are two main types of horizon sensors: statics and scanning. A static horizon sensor is able to detect the infrared radiation emitting from the Earth’s surface to locate the horizon of Earth. Scanning horizon sensors are much more complicated and use a system consisting of a spinning mirror to direct light onto a bolometer. This bolometer can sense when the infrared signal is present or lost. So, as the mirror rotates and reflects infrared light into the bolometer, the bolometer is able to detect when the signal is present or lost to determine the edge of the horizon .   Example Horizon Sensor      Deep Space  As explained earlier, in deep space it is possible to obtain a vector to the Moon to be used in the attitude determination algorithm. The Moon sensor would give a vector to the Moon in the body frame while an inertial vector would be needed . This inertial Moon vector could be obtained via the Moon's ephemeris data which could be loaded onto the satellite's processor and use the orbital elements of the Moon to determine its position relative to the Earth. However, the Moon's ephemeris data would more than likely give the Moon's position relative to the Earth ( ). The vector would then be given by   where is the satellite's position relative to the Earth. Note however that the position of the satellite relative to the Earth would need to be obtained via the Deep Space Network (DSN) and a combination of state estimation by integrating the orbital equations. The reference paper is a great paper that details all the different kinds of sensors and their algorithms. This section will eventually be supplemented by the material in that reference paper.   "
},
{
  "id": "fig-magnetometer",
  "level": "2",
  "url": "sec-spacecraft-attitude-determination.html#fig-magnetometer",
  "type": "Figure",
  "number": "20.4.1",
  "title": "",
  "body": " Example Magnetometer   "
},
{
  "id": "fig-rate-gyro",
  "level": "2",
  "url": "sec-spacecraft-attitude-determination.html#fig-rate-gyro",
  "type": "Figure",
  "number": "20.4.2",
  "title": "",
  "body": " Example IMU with Integrated Rate Gyro   "
},
{
  "id": "fig-accelerometer",
  "level": "2",
  "url": "sec-spacecraft-attitude-determination.html#fig-accelerometer",
  "type": "Figure",
  "number": "20.4.3",
  "title": "",
  "body": " Example Accelerometer   "
},
{
  "id": "fig-star-tracker",
  "level": "2",
  "url": "sec-spacecraft-attitude-determination.html#fig-star-tracker",
  "type": "Figure",
  "number": "20.4.4",
  "title": "",
  "body": " Example StarTracker   "
},
{
  "id": "fig-sun-sensor",
  "level": "2",
  "url": "sec-spacecraft-attitude-determination.html#fig-sun-sensor",
  "type": "Figure",
  "number": "20.4.5",
  "title": "",
  "body": " Example Sun Sensor   "
},
{
  "id": "fig-horizon-sensor",
  "level": "2",
  "url": "sec-spacecraft-attitude-determination.html#fig-horizon-sensor",
  "type": "Figure",
  "number": "20.4.6",
  "title": "",
  "body": " Example Horizon Sensor   "
},
{
  "id": "sec-position-estimation",
  "level": "1",
  "url": "sec-position-estimation.html",
  "type": "Section",
  "number": "20.5",
  "title": "Position Estimation",
  "body": " Position Estimation  Position estimation is for determining the placement or location of the spacecraft in orbit. This can often be confused with attitude estimation, however, this can be clarified with an analogy. A human's posture is their attitude while their location, or place in reference to something else, is their position. This analogy can be transferred to spacecraft attitude and position. Position estimation is how mission control determines the location of the spacecraft in space. It is vital for the spacecraft to know its position in order for the on board sensors to work as intended. Position estimation is similar to attitude control in the sense that the orbit plays a large role in determining the proper component. The major options for attitude estimation are Global Positioning Systems and the Ground Station Network.  The Global Positioning System (GPS) was developed in order to allow accurate determination of geographical locations by military and civil users. It works by using satellites in Earth’s orbit to transmit data which makes it possible to measure the distance between the satellites and the operator. This form of signal communication is incredibly accurate and used heavily for attitude estimation. Up to 30 GPS satellites are currently in orbit, mostly in MEO, at altitudes around 20,000 km. There will be between four and eight of them above any site on the Earth at any time . These satellites continuously emit coded high-frequency radio signals which may be received by special GPS receivers. These signals contain information about the exact orbits of the satellites and the time of atomic clocks onboard. When signals from three or more satellites are received, the GPS receiver will compute the best possible location of the user by triangulation. Much like when on Earth, a GPS can be used for navigation in space. The GPS receiver on board the satellite will also receive its longitude, latitude, and altitude as long as it is within the GPS constellation.   Example GPS Receiver    "
},
{
  "id": "fig-gps-receiver",
  "level": "2",
  "url": "sec-position-estimation.html#fig-gps-receiver",
  "type": "Figure",
  "number": "20.5.1",
  "title": "",
  "body": " Example GPS Receiver   "
},
{
  "id": "sec-trade-studies",
  "level": "1",
  "url": "sec-trade-studies.html",
  "type": "Section",
  "number": "20.6",
  "title": "Trade Studies",
  "body": " Trade Studies  Tradeoffs and considerations are analysis tools used to compare different components. It is not always obvious which component is best for a subsystem function. As shown in the previous sections, there are several options for attitude control, attitude estimation, and position estimation. It may not stand out at first glance which components are the best, so it is up to the team to perform a proper tradeoff analysis, a.k.a a trade study. What this typically looks like is a comparison of the different parameters of the components. For example, if the GNC team decides to look at an integrated system for attitude estimation, a trade study can be conducted to compare the mass, power, volume, and momentum storage for different Blue Canyon Technologies XACT systems . It is important to keep this trade study organized and regularly updated, so it is recommended that a table is created to compare these parameters.  The name “tradeoff” comes from the fact that for a system it is typical that one feature must be compromised for the other. For example, to get more power, it is probably reasonable to assume that volume will be compromised because the size of the component will need to increase to fit a larger power supply. Another example may be that in order to increase the volume of a component to fit a larger payload, the mass will be compromised and may increase. These compromises especially become issues when they interfere with the subsystem requirements.  "
},
{
  "id": "sec-risks",
  "level": "1",
  ...
  "number": "20.7",
  "title": "Risks",
  "body": " Risks  Risk assessments are arguably the most important task for the GNC team. It is crucial that the entire system is taken into consideration and assessed for possible failures because only one part needs to fail to cripple the whole mission. This includes non physical issues such as increased lead times on parts for example. The purpose of a risk assessment is to identify all possible risks, their severity and likelihood and to come up with mitigation strategies for those risks.  The purpose of risk assessments is to determine mitigation strategies so that the identified risks may be prevented. These mitigations are discussed amongst the team and heavily considered while designing the subsystem. Mitigation strategies can range anywhere from simply creating a preflight checklist to diverting control from one component to another in the event of a failure. These strategies are the main purpose of creating a risk analysis in the first place, so careful deliberation should be used when creating them.  The best way to conduct a risk assessment is by creating a risk table for the GNC subsystem. A risk table will list out the possible risks determined by the team and their subsequent mitigation strategies. It will also list out the likelihood and severity of each individual risk as decided by the team. These can be determined in any way deemed reasonable by the team, but is most often used on a scale of one to five for each category. These values can then be input into a risk criticality matrix which shows which risks are of the greatest threat level.  It is important that risk statements are written in a specific way to avoid any confusion or miscommunication between the different subteams. A risk statement consists of four parts: the condition, departure, asset and consequence. The condition is a single phrase that describes the current key fact-based situation or environment that is causing concern, doubt, anxiety, or uneasiness. The departure describes a possible change from the design or plan. It is an undesired event that is made credible or more likely as a result of the condition. The asset is an element of the system or plan. It represents the primary resource that is affected by the individual risk. The consequence is a single phrase that describes the foreseeable, credible negative impact(s) to meet performance requirements. A proper risk statement should utilize these four parts in a format similar to the following: “Given that [CONDITION], there is a possibility of [DEPARTURE] adversely impacting [ASSET], which can result in [CONSEQUENCE]”.  "
},
{
  "id": "sec-conclusions",
  "level": "1",
  "url": "sec-conclusions.html",
  "type": "Section",
  "number": "20.8",
  "title": "Conclusions",
  "body": " Conclusions  The GNC subsystem's purpose is to know where the spacecraft is, where it needs to go, and determine how it will get there. These requirements are determined by the overall mission goal of the spacecraft. The system constraints are divided into three main categories: the attitude control, attitude estimation, and position estimation. The attitude control is how the spacecraft will maintain its current orientation during orbit. Magnetorquers, reaction wheels, thrust vector control, reaction control systems, and control moment gyros are all common components used to counteract the disturbance torques and maintain the desired orientation. Attitude estimation is how the spacecraft estimates its current orientation. This is commonly achieved by star trackers, sun sensors, magnetometers, rate gyros, accelerometers, and horizon sensors. Lastly, the position estimation is knowing the precise location of the satellite during its orbit. GPS and the GSN are the two most common methods for determining the satellites position.  Not only are the components important but one must also consider the trade-offs and risks associated with the mission. Risks must be assessed and properly mitigated in order to ensure the safety and success of the spacecraft. This is done by creating risk tables using if-then terminology.  "
},
{
  "id": "sec-vehicle-selection",
  "level": "1",
  "url": "sec-vehicle-selection.html",
  "type": "Section",
  "number": "21.1",
  "title": "Vehicle Type Selection and Requirements",
  "body": " Vehicle Type Selection and Requirements  In the very beginning of your design you need to decide on the type of aircraft you want to build. Designing a glider versus an aerobatic airplane will result in vastly different engineering design decisions. For example, a glider is going to have very long slender wings while an aerobatic airplane is going to have somewhat shorter wings with large control surfaces. I suggest you select from the following types of aircraft and then move on: Gliders, Trainers, Sport Aerobatic, Racers. If you'd like to build a scale aircraft there isn't much to design since the shape of the aircraft is pretty much built. If you do go with a scale aircraft this textbook isn't really for you since you aren't really building a scratch build aircraft. You're more just copying someone else's design. In that case you may as well just buy a kit or watch some videos on balsa wood construction and an overview of all the electronics required for RC aircraft flight.  If you selected one of the other styles you're ready to move onto to the next stage which is requirements. There is so much literature on Systems Engineering, top level requirements, functional requirements and derived requirements. The bottom line is you need to determine what you want your aircraft to do. Do you want it to fly straight up, upside down? Do you want the aircraft to be hand launched? Land on a runway? Determine what you want the aircraft to do and create a bulleted list of those requirements. Throughout the design you can refer to these requirements and make sure you are satisfying these requirements. If this is your first build then you may just have one requirement and that is to take off and land without crashing. But think a bit deeper. Do you want to turn the vehicle? Do you want full channel control for roll, pitch and yaw or just yaw control? Do you want landing gear? What sort of flying characteristics do you want? Be as specific as possible here.  "
},
{
  "id": "sec-initial-design",
  "level": "1",
  "url": "sec-initial-design.html",
  "type": "Section",
  "number": "21.2",
  "title": "Initial Design - Hand Sketch and Aspect Ratio",
  "body": " Initial Design - Hand Sketch and Aspect Ratio  Once you have an idea of what the aircraft type is and what the requirements are it's time to hand sketch your aircraft. Try and use engineering paper, french curves, a ruler and a compass. Make this hand sketch look nice so you can use it in your future design. Draw your sketch to scale. You might be wondering, \"how do I draw the aircraft without knowing what my wing loading or thrust to weight ratio is?\". The answer comes from my late aircraft design professor Dr. Mikolowski (RIP). He would always say \"If it looks good, it flies good\". After designing so many aircraft and seeing so many scratch builds from my students I can honestly say that this is true 100%. If you're reading this now it means that there is already over 100 years of aircraft technology on the internet for you to research and see what other aircraft look like. Make your aircraft look like that but make sure it fits into your aircraft type and make sure it satisfies your requirements from above. If one of your requirements was to hand launch, then make sure your drawing reflects a vehicle without landing gear and a place to grab the aircraft. If you wanted full channel support make sure to include all the control surfaces. Think about where you want the propulsion system to go and how you're going to access the electronics before you fly. Think about what you want the wing to look like. Make it as big as you think it needs to fly. Use your intuition. This is an art. So much of engineering is an art.  Once your aircraft sketch is complete (make sure to do a front view, side view and top view of your aircraft), it's time to take down some wing characteristics. This is why you need to draw your sketch to scale. Measure the length of the wing (wingspan ) and the chord at the root ( ) and the tip ( ). Compute the area of the wing ( ) using the area of a trapezoid or rectangle depending on the shape of your wing. Once you have the wingspan and area you can compute the aspect ratio of your aircraft.   The general rule is that the larger the aspect ratio the more aerodynamically efficient your aircraft will be. This is why gliders have very long and slender wings. At the same time, high aspect ratio wings suffer from larger bending moments and can flex considerably in flight. Finally, it's important to compute the mean aerodynamic chord of the wing. This is basically the average chord of your wing. If you create a rectangular wing your mean aerodynamic chord is just the chord of the wing since it's constant. If not you'll need to integrate over the length of the wing using the formula below where y = 0 is the centerline of the vehicle and y=b\/2 is the wingtip on the right side . The parameter is the chord length as a function of y.   "
},
{
  "id": "sec-weight-estimate",
  "level": "1",
  "url": "sec-weight-estimate.html",
  "type": "Section",
  "number": "21.3",
  "title": "Weight Estimate - Tabular Approach",
  "body": " Weight Estimate - Tabular Approach  Once you have an idea of the overall shape it's time to estimate how heavy the aircraft will be. First think about the fundamental components of an aircraft. If you're not familiar with any of the components below, just type the item into Google and you'll find numerous articles and Youtube Videos about each component.   ESC - Electronic Speed Controller  Battery - Assume for now that you'll be using a 1500mAh 3S Battery unless you're building a micro aircraft in which case you might end up using a 600 mAh 2S or even a 300 mAh 1S. Think about the size of your aircraft. You will do more sophisticated battery design in the future.  Motor and Propeller - Again select something that is in the ballpark of the aircraft you're building. You'll do a redesign later.  Servos  Receiver  Control Linkages and Servo Horns  Fuselage  Main Wing  Tail both Horizontal and Vertical  Payload   For each of the components above you need to estimate the weight of these components. The only way to do that is to either look up the weight of similar aircraft to the one you're designing or find components that you think will work for your aircraft and add up all of the weights. The most difficult part is going to be estimating the empty weight of the aircraft which is just the structure of the aircraft. For these estimates you need to decide what materials you plan on using. Is your aircraft made out of foam, balsa, carbon fiber or some type of combination. Perform an initial material selection and then use that to estimate your weight. Create a table in a spreadsheet type program or a numerical computer program so that you can go back and change weights as your design progresses. Use the spreadsheet or numerical program to compute the total weight of your aircraft. This is your maximum takeoff weight.  "
},
{
  "id": "sec-airfoil-selection",
  "level": "1",
  "url": "sec-airfoil-selection.html",
  "type": "Section",
  "number": "21.4",
  "title": "Airfoil Selection and 2D and 3D Lift",
  "body": " Airfoil Selection and 2D and 3D Lift  In this stage of design we will only be looking at the lift equation.   The coefficient of lift is the lift coefficient of the aircraft. This coefficient is a function of Reynolds number, angle of attack, airfoil shape and wing shape. Recall that the angle of attack is the angle between the zero lift line of the airfoil and the free stream air. Breaking the velocity vector into components yields the equation below where is the airspeed along the z-axis of the vehicle and is the velocity along the x-axis.   Using the angle of attack as a parameter, the lift coefficient can be expanded to the following:   The parameter is the angle of attack that results in zero lift. This is a function of the airfoil shape. The parameter is the lift curve slope and is a function of airfoil shape, wing shape and Reynolds number. In order to remove, wing shape from the design, the aspect ratio is used to convert the wing lift to a sectional airfoil coefficient.   The parameter is an efficiency parameter which is often assumed to be 80-90%. The coefficient, is the airfoil lift curve slope which is different than the wing lift curve slope. The wing lift curve slope will always be smaller than the airfoil lift curve slope. This is because of an effect called wing tip vortices. Wing tip vortices are an aerodynamic effect where high pressure from the bottom of the wing moves around the wingtips to the area of low pressure. The only way to mitigate these vortices is by installing winglets or increasing the aspect ratio. Note that winglets increase drag and weight which is why you don't typically see them on RC aircraft. The coefficient is then simply a function of airfoil shape and Reynolds number. This is what is often referred to as 2D lift. It is the lift of airfoils which are two dimensional rather than 3D lift which is over an entire wing. It is at this point that airfoil selection and design can be used. The website airfoiltools.com is a great resource for plotting lift curve slopes of various airfoil shapes as a function of Reynolds number. I also have a great Youtube video on how to use XFLR5 (pronounced X-Flyer Five) . A more comprehensive guide on XFLR5 can be found in while the software itself can be downloaded in . The basics of airfoil selection then break down into the following process.  First, using your max takeoff weight and cruise flight speed, compute the lift coefficient required in cruise. This assumes that lift equals weight.   Then using your flight speed and chord length, compute the Reynolds number of your aircraft. Use this Reynolds number and airfoiltools or XFLR5 to compute the sectional lift characteristics of various airfoils. Airfoil selection can be as complicated as you make it but you're looking for the highest lift to drag ratio airfoil. Another simple way is to just select the airfoil with the highest sectional lift coefficient. Another item to consider is manufacturing. Some airfoils may be feasible for full sized aircraft but not for RC flyers. My recommendation is to build an aircraft with a Clark-Y airfoil. They are easy to cut with balsa or shape with foam and have good lift to drag characteristics. Note that this airfoil is cambered. If you want to fly upside down I suggest you use a symmetric airfoil like a NACA 0012 or NACA 0014 is you need a bit more thickness to fit a wing spar through the airfoil.  Once you have your airfoil selected, use the lift curve slope to estimate by fitting a linear trend line to the portion of the graph before stall. Also make sure to take note of what the zero lift angle of attack is. You can then compute the wing lift curve slope by using equation . Once you have , you can compute the angle of attack needed for cruise. Make sure to convert to radians before you use the equation below.   The resulting answer will be in radians and needs to be converted to degrees to make sure you are not close to stall. Typically in cruise, the angle of attack is only a few degrees with perhaps 8 degrees on the high end. If the answer you receive is higher than that it can mean a few things which may involve a redesign.   The simplest way to get more lift is to fly faster. The problem is you need a bigger motor which will increase your weight which will require more lift and more angle of attack. Flying faster will also change your Reynolds number.  You can increase the aspect ratio of your wing which will make your aircraft more efficient. The problem is that will increase the bending moment at the root creating the need for stronger materials at the root which also increases weight. This will also change your mean aerodynamic chord which will change your Reynolds number.  You can increase the area of the wing. This will also increase drag and weight but if the material you are using has a high lift to weight ratio then adding more wing area might be a good option. Depending on how you change the aircraft wing shape, the aspect ratio and\/or the mean aerodynamic chord might change meaning you'll have to recompute the wing lift curve slope as well as the Reynolds number.  If you are using a symmetric airfoil it's possible you could forgo flying upside down and switch to a cambered airfoil which has more lift.   Regardless of what you do make sure you angle of attack in cruise is low which will reduce drag. It's optimal to fly at the aircraft's highest lift to drag ratio but radio controlled aircraft don't typically do that. It is also important to compute the stall speed of your aircraft. You'd like this value to be as small as possible.   In this case is the maximum lift coefficient your aircraft can obtain before stalling. If the stall speed is too high for your design go back and redesign your vehicle. Once you have redesigned your vehicle to fit within tolerable limits it's time to look at some aircraft performance characteristics which include the W\/S (wing loading) and the T\/W (thrust to weight ratio).  "
},
{
  "id": "sec-wing-loading",
  "level": "1",
  "url": "sec-wing-loading.html",
  "type": "Section",
  "number": "21.5",
  "title": "Wing Loading and Thrust to Weight Ratio",
  "body": " Wing Loading and Thrust to Weight Ratio  The wing loading is defined as the weight of the aircraft over the main wing area (W\/S). Intuitively though, it is the amount of lift required per square foot of wing area for your aircraft. If the wing loading is high it means you have a heavy aircraft with small wings which means you either need very high lift creating devices like flaps and cambered airfoils or the aircraft needs to fly very fast. You can imagine that warbirds and racers have higher wing loading then say a glider which has very low wing loading. In this case the aircraft can fly slow because the aircraft is light with larger wings. At this stage of the design you already know the maximum weight of the aircraft and the main wing area so it's simple to calculate. For larger aircraft, there is a standard wing loading for aircraft as well as another parameter called the wetted wing loading which is the weight divided by the wetted area of the wing. The wetted area is basically the surface area of the wing. For radio controlled aircraft though the wing cube loading is used to ensure the aircraft is of the correct type.   Using the units of ounces for the weight and sqft for the area the table below can be used to ensure that your aircraft is in the correct ballpark .     Type of Aircraft WCL (oz\/ft )  Gliders under 4  Trainers 5-7  Sport Aerobatic 8-10  Racers 11-13  Scale over 15     After computing your WCL it is possible that for the type of aircraft you've designed, your value falls well outside the limits of the table above. In that case you must redesign your vehicle by changing the shape of the wings or choosing a different material to lighten up the aircraft. In my experience, if you are designing a racer or scale aircraft and you wing loading is smaller than above this is typically ok so long as you have enough thrust to fly fast. In this case you may just have a very fast trainer which isn't necessarily a bad thing. The danger is when trying to design a glider with a WCL of over 15. That aircraft will exhibit a very high stall speed and poor lift to drag ratio (L\/D) characteristics. Once you are satisfied with your WCL you can move on to computing the thrust to weight ration (T\/W).  For full-sized aircraft the T\/W can range from 0.6 for passenger aircraft all the way up to 1.2 or higher for get aircraft. For R\/C aircraft the same general rule applies. If your aircraft is a trainer with landing gear you can probably get away with a T\/W of 0.6 but I would not recommend it. My recommendation would be to go no lower than 0.8 which would mean your maximum take off thrust is 80% of your maximum takeoff weight. In this configuration, the aircraft will accelerate down the runway until just over stall speed at which point the aircraft can takeoff. If you have strict runway requirements, airborne requirements like vertical flight or loops and snap rolls, I recommend increasing the size of your motor, ESC, battery, propeller combination to yield a T\/W of at least 1.2. In this case, even if your wings are not very efficient, you can fly on thrust alone. You may not exhibit great aerodynamic performance and still have a high stall speed but worst case you can land the aircraft like a harrier which I've done before.  Once you've selected your T\/W you need to go find a battery\/ESC\/motor\/propeller combination that yields the thrust you need. Tiger Motors website is typically very good at listing the motor, propeller and battery combination to give you a certain amount of thrust. Unfortunately, the hobbyist market is not using standard engineering units and thrust is reported in grams. My recommendation then is to use the following formula to compute your thrust required in grams. This assumes that 4.44 N = 1 lbf and that you are on Earth with 9.81 m\/s of gravitational acceleration. It'd be nice if the community just reported thrust in lbf but alas that is not the case.   Remember, when selecting a propulsion system, you will need to go back and update your weight estimate with the new values you've obtained. This may effect your wing area slightly and may even require you to choose a new motor if you were very off the first time you estimated the weight. This is an iterative process and every step builds on the previous step. The hope is that each iteration is not very different than the last.  "
},
{
  "id": "sec-wing-loading-5-1",
  "level": "2",
  "url": "sec-wing-loading.html#sec-wing-loading-5-1",
  "type": "Table",
  "number": "21.5.1",
  "title": "",
  "body": "  Type of Aircraft WCL (oz\/ft )  Gliders under 4  Trainers 5-7  Sport Aerobatic 8-10  Racers 11-13  Scale over 15   "
},
{
  "id": "sec-battery-sizing",
  "level": "1",
  "url": "sec-battery-sizing.html",
  "type": "Section",
  "number": "21.6",
  "title": "Battery Sizing",
  "body": " Battery Sizing  For battery sizing, you need to make sure the battery voltage is not too high for the ESC or the motor. Typically the ESC and motor will have a voltage limit listed on there. For example, an ESC that can handle 3-6S LiPos can handle anywhere from 12.6V to 25.2V. Motors may have different constraints than the ESC. It's also important to make sure that you look up the maximum current in the motor and that number must be less than the maximum current rating of the ESC. To compute flight time at max throttle use the equation below:   where is the number of milliamp hours of your battery, is the max current draw of your motor at full throttle and is the time of flight in hours. Note that you can repeat the calculation for different throttle settings provided you know the current through the motor.  You also need to make sure that the battery can output the necessary current your motor needs at full throttle. The equation below is used to ensure the battery can supply the necessary current.   In the equation above, is the C rating of the battery usually listed on the battery itself or in the product description.  "
},
{
  "id": "sec-stability-control",
  "level": "1",
  "url": "sec-stability-control.html",
  "type": "Section",
  "number": "21.7",
  "title": "Stability and Control, Center of Mass, Aerodynamic Center and Static Margin",
  "body": " Stability and Control, Center of Mass, Aerodynamic Center and Static Margin  Stability and Control is a very large section of literature and could be taught over an entire semester. Stability just ensure the aircraft flies steady and level and is typically broken up into lateral (side to side) and longitdutinal (front to back) stability. Lateral stability in my opinion is more complex but to ensure you aircraft is laterally stable, just be sure your aircraft is symmetric about the left and right planes and also be sure that your tail surface provides adequate yaw stability through the use of a vertical tail or a V-tail if you opted for a combined control surface. Longitudinal stability involved two more calculations that must be done before the aircraft can be built. These two parameters are the center of mass and the aerodynamic center. The center of mass is a very simple quantity to compute by just using the center of mass formula shown below.   In the equation above, is the distance of a component from a reference point on the aircraft. I typically use the nose of the fuselage as the reference point. For standard aircraft the motor would have a negative distance from the reference point and the receiver and battery would have a positive value. The value is then the weight of each component. Placing servos, receivers and other electronics is a design parameter to move your center of mass while the fuselage is typically a parameter that must be estimated at this stage. My recommendation is to break the aircraft into fuselage, tail boom, tail and main wing components and treat each one as a component. You will notice that moving the main wing and battery drastically changes the center of mass.  The next parameter is the aerodynamic center. The main wing looking from the top is basically a 2D distributed load. As such the center of lift must be computed. Assuming the main wing is symmetric, the aerodynamic center will lie on the center line of the aircraft. In this case the problem reduces to a 1D computation. In order to compute the center of lift along the x-axis (pointing towards the nose) you need to compute the weighted average of the center of lift of each airfoil. In this case if you have a symmetric airfoil, the center of lift is 1\/4 of the chord length. If you have a cambered airfoil the center of lift is typically in the 30% range so you can use 25% for a symmetric airfoil and something slightly larger for a cambered airfoil. Once you determined the center of lift for the airfoil you can use the equation below for the aerodynamic center of the entire wing , .   The parameter is the location of the center of lift of the airfoil as a function of y. Once you have the aerodynamic center and the center of mass you can compute the static margin of your aircraft.   The value of the static margin is not as important as the sign. If measuring from the nose of the aircraft the aerodynamic center must be behind the center of mass. To explain this think about stable aerodynamic vehicles like darts, or arrows. Notice that darts and arrows have fletching in the rear to create aerodynamic surfaces farther back. Unstable vehicles like frisbees and footballs have aerodynamic centers in front of the center of mass in which case they must spin in order to provide stability just like a bike tire or a dreidel. Using the equation above, if is behind it means that is bigger or more positive than in which case your static margin would be positive.   If you perform these two calculations and find your static margin to be negative it means that you need to shift your battery and other components more towards the nose or move your wings backwards. Note that moving your wing backward will also shift your center of mass so try and move some components forward before shifting your wings around.  The final stage of this design is Control. Aircraft in flight require 3 control surfaces to provide roll, pitch and yaw control. These include the aileron, elevator and rudder. It's possible to fly aircraft without a rudder by performing a \"bank and yank\" maneuever and it's also possible to combine elevators and ailerons into something called elevons. I've even seen some ruddervators. Whatever you decide to do make sure that you can adequately control all three axes or if one axis is uncontrollable be sure that that axis is stable. I've seen some aircraft that only have rudder and elevator and no ailerons. I find these aircraft hard to control but the idea is you move the rudder to yaw the aircraft which also rolls the aircraft allowing you to turn. It creates a very slow aircraft but it also reduces complexity if that is something you're interested in doing.  "
},
{
  "id": "sec-iteration-sketch",
  "level": "1",
  "url": "sec-iteration-sketch.html",
  "type": "Section",
  "number": "21.8",
  "title": "Iteration, Detailed Sketch and Final Checks",
  "body": " Iteration, Detailed Sketch and Final Checks  This section in my opinion is absolutely essential. It involves going back and making sure that your current design satisfies your requirements you originally wrote in the first section of this design. Recompute your aspect ratio, and update your weight estimate based on any calculations you've obtained. This may be finding better estimates for parts or materials. You also need to create a better sketch and determine where EVERY component is going to go and what sort of support you will need. If you're building your aircraft out of balsa you will need detailed sketches on rib, spar and stringer placement. All of these updates will change your weight estimate which will change your WCL and your T\/W. Be sure your WCL and T\/W are within tolerable bounds. You also need to go back and compute you angle of attack during cruise and be sure you are not in a stall regime. Be realistic with your flight speed as well during cruise. Go back and compute your stall speed. Is it realistic? If not then go back and make some minor changes. Finally, be sure you aircraft is longitudinally and laterally stable and that you can control all 3 axes or at least the uncontrollable axes are stable. Once you are certain the aircraft will fly you can begin purchasing components.  "
},
{
  "id": "sec-cad",
  "level": "1",
  ...
  "body": " Computer Aided Design (CAD)  This section is optional but sometimes it's just nice to have a CAD view of your aircraft especially if you are 3D printing parts or perhaps getting some component machined out of aluminum. Some CAD programs are even so powerful they will compute bending loads, center of mass and even drag. Use whatever tools are at your disposal to help you in the design.  "
},
{
  "id": "sec-purchase-components",
  "level": "1",
  "url": "sec-purchase-components.html",
  "type": "Section",
  "number": "21.10",
  "title": "Purchase Components",
  "body": " Purchase Components  I wanted to write an entire section on purchasing component to go over a few common mishaps. First, if you are using a LiPo battery be sure to familiarize yourself with the dangers of LiPo batteries. I've almost caught my entire lab on fire by charging a damaged LiPo but all batteries can technically catch fire. Please be careful. Furthermore, be sure you purchase an ESC with the right current rating. If you overload an ESC with too much current it will also catch on fire. The servos you buy have a torque rating. Be sure your servos can overcome the aerodynamic torque estimated in flight. Generally servos are sized by the size of the aircraft so you can just purchase servos that are designed for your particular size aircraft. The motor you purchase is going to need a mounting point. Consider designing a motor plate or even a firewall depending on the type of aircraft. When purchasing materials make sure to get them from a good brand. Companies like Flite Test sell really good double plated foam that is designed for RC aircraft. Purchasing Dollar Tree foam board is totally acceptable but just understand that after a crash or two the foam board won't work anymore. Also consider what type of glue you are planning on using. Certain types of glue can actually melt foam and hot glue can melt certain types of foam as well. CA glue is very good for balsa but will melt foam. Two part epoxy is strong but it weighs more than CA glue and takes a long time to set compared to other types of glue. Also be sure to be lenient on glue where you can. Glue just adds weight and that will reduce your performance. Finally, make sure your receiver supports the number of servos you plan on using and be sure that your transmitter and receiver are compatible. All of my aircraft use Spektrum technology but you may opt to use a different type of protocol.  Finally, when all components come in be sure to test them. DOA stands for dead on arrival and so many components come DOA. Before you spend the time to install all your components in your aircraft and then throw the aircraft in the air make sure you test every component and be sure it works. I also suggest weighing each component on a small scale and updating your weight estimate to ensure everything is within tolerable bounds.  "
},
{
  "id": "sec-building",
  "level": "1",
  "url": "sec-building.html",
  "type": "Section",
  "number": "21.11",
  "title": "Building",
  "body": " Building  Once you have all necessary resources to build your aircraft I recommend starting with the fuselage or main wing and then installing all componets. I recommend taking pictures of your build in case you need to reference them later. Go slow. If you break something it will be expensive. Also remember that if the aircraft looks good it flies good. This means that gluing all components properly and having the aircraft be as smooth as possible will translate to better flight performance.  "
},
{
  "id": "sec-flying",
  "level": "1",
  ...
  "number": "21.12",
  "title": "Flying",
  "body": " Flying  Before you fly I recommend finding a pilot will more experience than yourself to check out the aircraft and make sure the aircraft has been built properly. You may even want to discuss your initial design before you even begin building to make sure there are no major critical issues. If you want to fly the aircraft yourself I suggest flying an aircraft using a simulator. My recommendation is the free program CRRCSim . It is not a great simulator by any means but it will at least familiarize yourself with aircraft controls. Before you fly make sure to build yourself a pre and post flight check list. I've included my pre and post flight check list that I use before every flight test.   Day Before Flight Checklist    Assess the weather to ensure acceptable flight conditions   No strong winds (anything over half your stall speed is too fast)  No rain or lighting    State and confirm the purpose of the flight test - Set clear goals the aircraft should complete before test  Check for damage to the plane and if the moving parts are secured including motor and electronic speed control and all components  Check for a full battery charge on plane and controller; charge all electronics if not fully charged  Perform Ground Safety Check List  Take note of items that need to be repaired even if the flight test is not implemented    Ground Safety Check List    Ensure that propellor is off  Turn on TX  Connect battery to aircraft  Ensure all control surfaces are operational and moving the correct way  Spin up motor and be sure that motor is spinning the correct way  Perform a range check where pilot moves control surfaces with 50% or more throttle while walking away from aircraft. Ensure that pilot can move at least 300 feet away without any dropouts.  Remove battery and install propeller  Reconnect battery.  Using safety glasses, apply full throttle to TX and ensure that adequate thrust is generated to fly aircraft. Leave full throttle applied for at least 30 seconds to be sure no component fails. Better to fail on the ground than in the air.    Preflight Checklist    Perform all “Day Before Flight” Checks  Perform Ground Safety Checks  Check for any damage to any components including the battery  Install Prop  Turn on TX  Plug in main battery  Confirm flight time and range distance  Clear obstructions and make sure there is clear space for takeoff and landing  Arm TX if the TX has an arm switch  Ensure all control surfaces are operational and moving the correct way  Apply throttle and fly  Upon landing do everything in reverse order    Post Flight checklist    Check plane for any damage  Check all moving parts are still secured  Check for battery overheating, discoloration, warping, or swelling  Check battery usage with a voltmeter  Check if the plane is able to power on again  Have PIC (pilot in command) give a post flight assessment  Put batteries in LiPo storage   "
},
{
  "id": "sec-helpful-aircraft-equations",
  "level": "1",
  "url": "sec-helpful-aircraft-equations.html",
  "type": "Section",
  "number": "21.13",
  "title": "Helpful Aircraft Equations",
  "body": " Helpful Aircraft Equations   This section provides a collection of essential equations utilized in aircraft design and performance analysis. It covers fundamental relationships for atmospheric conditions, aerodynamic force and moment coefficients, non-dimensional parameters, and flight mechanics.    Mach Number and Reynolds Number       Total Velocity       Angle of Attack and Sideslip       Lift Drag and Moment       Lift and Drag Coefficients       Non-dimensional Angular Velocities       Pitch Moment Equation          Max Lift to Drag Ratio (Only valid if )       Lift to Drag when (Sum of Forces still zero)       Airfoil and Wing Aerodynamics       Standard Atmosphere       General Notes    In trim or steady and level or cruise , , , .    For symmetric airfoil and thus .    For a flat plate all symmetric properties apply but .    Tail surfaces are always assumed to be flat plates.    For longitudinal problems, so (side velocity).     "
},
{
  "id": "conclusions",
  "level": "1",
...
},
{
  ...
  "number": "",
  "title": "References",
  "body": " References  GitHub. GitHub Copilot. https:\/\/copilot.github.com\/, 2021. Accessed on December 5, 2025.  Google DeepMind. Gemini: Googles multimodal ai model. https:\/\/gemini.google.com\/, 2023. Accessed on December 5, 2025.  Wikipedia. V-model. https:\/\/en.wikipedia.org\/wiki\/V-Model. Accessed December 21st, 2021.  Draw.io. JGraph Ltd, 2026, www.drawio.com\/. Accessed 29 Mar. 2026  Blender. Blender Foundation, 2026, www.blender.org\/. Accessed 29 Mar. 2026  Fritzing. Fritzing, 2026, fritzing.org\/. Accessed 29 Mar. 2026  SolidWorks. Dassault Systèmes, 2026, www.solidworks.com\/. Accessed 29 Mar. 2026  Autodesk. Autodesk, 2026, www.autodesk.com\/. Accessed 29 Mar. 2026  Cirkit Designer. Cirkit, 2026, https:\/\/app.cirkitdesigner.com\/. Accessed 29 Mar. 2026  Inkscape. Inkscape, 2026, www.inkscape.org\/. Accessed 29 Mar. 2026  CircuitPython. https:\/\/circuitpython.org\/, 2021. Accessed: 12-09-2021.  Carlos Montalvo, Lisa Schibelius, and Marine Leabeater. Project Based Engineering Instrumentation with CircuitPython. Github, 2023. https:\/\/cmontalvo251.github.io\/project-based-instrumentation\/.  Adafruit. Adafruit learn, 2021. Adafruit https:\/\/learn.adafruit.com\/.  Carlos Montalvo. Facility for aerial systems and technology github repository. cmontalvo251, 2022. https:\/\/github.com\/cmontalvo251  Adafruit. Adafruit, 2023. Adafruit https:\/\/adafruit.com\/.  Arduino. Arduino, 2023. https:\/\/www.arduino.cc\/.  Montalvo, Carlos. YouTube, 2026, www.youtube.com\/@CarlosMontalvo251.  Python Software Foundation. Python, 2026. https:\/\/www.python.org\/.  Aasumets, Aivar. Thonny: Python IDE for Beginners. 2026, thonny.org\/.  Spyder: The Scientific Python Development Environment. Spyder Project Contributors, 2026, www.spyder-ide.org\/.  Google Colaboratory. Google, 2026. https:\/\/colab.research.google.com\/.  Mu Editor. Mu, 2026. https:\/\/codewith.mu\/en\/.  GNU Octave. GNU, 2026. https:\/\/www.gnu.org\/software\/octave\/.  Google Sheets. Google, 2026. https:\/\/www.google.com\/sheets\/about\/.  B. Etkins. Dynamics of Atmospheric Flight. Dover, Mineola, New York, 2000. pages 9-13, 134-151, 196-318.  Warren F. Phillips. Mechanics of Flight. John Wiley and Sons, Hoboken, New Jersey, 2010.  R.C. Nelson. Flight Stability and Automatic Control. McGraw-Hill, 2nd edition, 1998.  Jerry E. White, Roger R. Bate, Donald D. Mueller. Fundamentals of Astrodynamics. Dover, 1971.  Arthur KL Lin and Regina Lee. Attitude control for small spacecraft with sensor errors. In AIAA SPACE Conferences and Exposition , Pasadena, California, page 4423, August 2015.  Nuno Filipe and Panagiotis Tsiotras. Adaptive position and attitude-tracking controller for satellite proximity operations using dual quaternions. Journal of Guidance, Control, and Dynamics , 38(4):566–577, 2015. doi:10.2514\/1.G000054.  O. A. Bauchau. Flexible Multibody Dynamics . Springer, 2008.  Karsten Grosekathofer and Zizung Yoon. Introduction into quaternions for spacecraft attitude representation. Technical University of Berlin Department of Astronautics and Aeronautics, 2012.  John L. Crassidis and John L. Junkins. Optimal Estimation of Dynamic Systems . Chapman and Hall\/CRC, 2004.  Sebastian Munoz and E. Glenn Lightsey. A sensor driven trade study for autonomous navigation capabilities. AIAA.  John L. Crassidis, F. Landis Markley, and Yang Cheng. Survey of nonlinear attitude estimation methods. Journal of Guidance, Control, and Dynamics , 30(1), 2007.  Bing Liu, Zhen Chen, Xiangdong Liu, and Fan Yang. An efficient nonlinear filter for spacecraft attitude estimation. International Journal of Aerospace Engineering , 2014(540235), 2014.  R. M. Georgevic. The solar radiation pressure forces and torques model. The Journal of the Astronautical Sciences , 27(1), 1973.  Anderson D. Fundamentals of Aerodynamics 4th Edition. McGraw Hill Series, 2007.  Graham Gyatt. The standard atmosphere. A mathematical model of the 1976 U.S. Standard Atmosphere, Jan 2006.  Geographic library c++. https:\/\/geographiclib.sourceforge.io\/. cited Jul 11, 2017.  L. G. Jacchia. Static diffusion models of the upper atmosphere with empirical temperature profiles. Smithson. Astrophys. Obs. Spec. Rept. , (170), 1964. Cambridge, Massachusetts.  L. G. Jacchia. Revised static models of the thermosphere and exosphere with empirical temperature profiles. Smithson. Astrophys. Obs. Spec. Rept. , (332), 1971. Cambridge, Massachusetts.  L. G. Jacchia. Thermospheric temperature, density, and composition: New models. Smithson. Astrophys. Obs. Spec. Rept. , (375), 1977. Cambridge, Massachusetts.  Enhanced magnetic field model. https:\/\/www.ngdc.noaa.gov\/geomag\/EMM\/. cited Jul 11, 2017.  Neil Ashby. The sagnac effect in the global positioning system. Relativity in rotating frames: relativistic physics in rotating reference frames , page 11, 2004. doi:1-4020-1805-3.  Earth gravity model. https:\/\/www.ngdc.noaa.gov\/geomag\/EMM\/. cited Jul 31, 2017.  SatCatalog. Satcatalog. cubeir - earth\/horizon sensor. https:\/\/www.satcatalog.com\/component\/cubeir\/. Accessed Nov. 4, 2021.  W. Emery and A. Camps. Introduction to satellite remote sensing: Atmosphere, ocean, land and cryosphere applications. Elsevier Science Publishing, 2017.  Chemical propulsion systems. glenn research center — nasa. https:\/\/www1.grc.nasa.gov\/research-and-engineering\/chemical-propulsion-systems\/l-propulsion-systems\/. Retrieved September 29, 2021.  Solid rocket propulsion. https:\/\/engineering.purdue.edu\/ propulsi\/propulsion\/rockets\/solids.html. Retrieved September 29, 2021.  H. Curtis. Magnetorquers: An overview of magnetic torquer products available on the global marketplace for space. https:\/\/blog.satsearch.co\/2019-08-21-magnetorquers-an-overview-of-magnetic-torquer-products-available-on-the-global-marketplace-for-space. (2021, July 22)  CubeSTAR. Cubestar. attitude determination & control. http:\/\/cubestar.no\/index.php?p=1_22_Attitude-Determination-Control:~:text=The%20detumbling%20is%20the%20process,derivative%20of%20the%20angular%20rate. Retrieved November 3, 2021.  Nctr-m002 cubesatshop.com. magnetorquer-rod\/. (2020, November 6). https:\/\/www.cubesatshop.com\/product\/nctr-m002-magnetorquer-rod.  Reaction wheels: An overview of attitude control systems available on the global marketplace for space. https:\/\/blog.satsearch.co\/2019-07-25-reaction-wheels-an-overview-of-attitude-control-systems-available-on-the-global-marketplace-for-space. Accessed Nov. 3, 2021.  NanoAvionics. Cubesat reaction wheels control system satbus 4rw.  Blue canyon technologies. https:\/\/www.bluecanyontech.com\/components. Retrieved November 29, 2021.  Thrust vector control - an overview — sciencedirect topics. https:\/\/www.sciencedirect.com\/topics\/physics-and-astronomy\/thrust-vector-control. Accessed Nov. 4, 2021.  Gimbaled thrust. https:\/\/www.grc.nasa.gov\/www\/k-12\/rocket\/gimbaled.html. Accessed Nov. 4, 2021.  Control devices. https:\/\/mars.nasa.gov\/mro\/mission\/spacecraft\/parts\/gnc\/controldevices\/. Accessed Nov. 4, 2021.  Low thrust, low impulse bit reaction control thruster. https:\/\/artes.esa.int\/projects\/low-thrust-low-impulse-bit-reaction-control-thruster. Accessed Nov. 4, 2021.  Sinclair D. Votel, R. and S. Interplanetary. Comparison of control moment gyros and reaction wheels for small earth-observing satellites.  B. L. Gyroscope.com. Control moment gyroscope platform-mk2. https:\/\/www.gyroscope.com\/d.asp?product=CMGMK2. Accessed Nov. 4, 2021.  Satellite star trackers - the cutting edge celestial navigation products available on the global space marketplace. satsearch blog. https:\/\/blog.satsearch.co\/2019-11-26-satellite-star-trackers-the-cutting-edge-celestial-navigation-products-available-on-the-global-space-marketplace. Accessed Nov. 3, 2021.  Spice toolkit. https:\/\/naif.jpl.nasa.gov\/naif\/toolkit.html. Accessed Nov. 3, 2021.  Advanced star tracker. https:\/\/www.esa.int\/Applications\/Telecommunications_Integrated_Applications\/Alphasat\/Advanced_Star_Tracker. Accessed Nov. 3, 2021.  What is a sun sensor? https:\/\/www.azosensors.com\/article.aspx?ArticleID=223. Accessed Nov. 3, 2021.  Nss cubesat sun sensor. CubeSatShop.com.  What is a magnetometer? sciencing. https:\/\/sciencing.com\/about-5397128-magnetometer.html. Accessed Nov. 4, 2021.  Satcatalog. hi-rel magnetometer fgm-a-75-magnetometer—satcatalog. https:\/\/www.satcatalog.com\/component\/hi-rel-magnetometer-fgm-a-75\/. Accessed Nov. 4, 2021.  Sensonor, “sensonor - stim300 sensonor.no.”. Accessed: 25-May-2021. https:\/\/www.sensonor.com\/products\/inertial-measurement-units\/stim300\/.  Industries, a. adxl345 - triple-axis accelerometer (+-2g\/4g\/8g\/16g) w\/ i2c\/spi. adafruit industries blog rss. https:\/\/www.adafruit.com\/product\/1231. Retrieved November 5, 2021.  Vincent L. Pisacane. Fundamentals of space systems. New York, Oxford University Press, 2005.  GPS.Gov: Space Segment. https:\/\/www.gps.gov\/systems\/gps\/space\/. Accessed Nov. 3, 2021.  Waslander S.L. Hoffman G.M., Huang H. and Tomlin C.J. Quadrotor helicopter flight dynamics and control: Theory and experiment. AIAA Guidance, Navigation, and Control Conference and Exhibit, pages 6–15, 2007.  H. Huang, G. Hoffman, C. Talor, S. Waslander, and C. Tomlin. Aerodynamics and control of autonomous quadrotor helicopters in aggressive maneuvering. Proc. IEEE Int. Conference of Robotics and Automation (ICRA), pages 3277–3282, May 2009. doi:10.1109\/ROBOT.2009.5152561.  Moosavian S.A.A. Sadr S. and Zarafshan P. Dynamics modeling and control of a quadrotor with swing load. Journal of Robotics, 2014.  Jet Propulsion Laboratory. Solar system dynamics. https:\/\/ssd.jpl.nasa.gov\/ Accessed October 4th, 2019.  Steven C. Chapra and Raymond P. Canale. Numerical Methods for Engineers. McGraw-Hill Education, 7th edition, 2012.  LLC. Horizon Hobby. Eflite Apprentice S 15e Airplane Instruction Manual. Horizon Hobby, LLC., Champaign, IL USA, 1 edition, 4 2016. Retrieved October 5, 2020 https:\/\/www.horizonhobby.com\/on\/demandware.static\/Sites-horizon-us-Site\/Sites-horizon-master\/default\/Manuals\/EFL2725-Apprentice-Manual.pdf.  Maxwell Cobar and Carlos Montalvo. The facility for aerospace systems and technology simulation: Fastsim- an open-source configurable software in the loop simulation environment. In AIAA AVIATION Forum -Chicago, IL, June 2022.  Maxwell Cobar and Carlos Montalvo. Takeoff and landing of a wingtip connected meta aircraft with feedback control. Journal of Aircraft - Q1, 58(4):733–742, 2021.  Maxwell Cobar and Carlos Montalvo (Chair). Waypoint control for a wingtip connected meta aircraft, December 2022.  Stephen M. Stigler. Gauss and the invention of least squares. doi:10.1214\/aos\/1176345451.  Lagrange, J. L. (Joseph Louis). Mécanique analytique. Paris, Ve Courcier, 1, 1811.  A. B. Younes, D. Mortari, J. D. Turner, and J. L. Junkins. Attitude error kinematics. Journal of Guidance, Control and Dynamics, 37(1), 2014  S. Caldwell. Ground data systems and mission operations. nasa. http:\/\/www.nasa.gov\/smallsat-institute\/sst-soa\/ground-data-systems-and-mission-operations. Accessed Nov. 4, 2021.  H. Monaghan. Networks. nasa. http:\/\/www.nasa.gov\/directorates\/heo\/scan\/services\/networks\/index.html.Accessed Nov. 4, 2021.  AmericaSpace. Happy birthday, dsn: Nasa’s iconic deep space communications network turns 50. https:\/\/www.americaspace.com\/2013\/12\/26\/happy-birthday-dsn-nasas-iconic-deep-space-communications-network-turns-50\/. Accessed Nov. 4, 2021.  Mirko Leomanni. Comparison of control laws for magnetic detumbling. Research Gate, 10 2012. accessed: 2\/9\/2018.  M. Lovera. Magnetic satellite detumbling: The b-dot algorithm revisited. In 2015 American Control Conference (ACC), pages 1867–1872, July 2015. doi:10.1109\/ACC.2015.7171005.  Farhat Assaad. Attitude Determination and Control System for a CubeSat. PhD thesis, Worcester Polytechnic Institute, March 1st 2013.  Amit Sanyal and Zachary Lee-Ho. Attitude tracking control of a small satellite in low earth orbit. Aug 2009. doi:10.2514\/6.2009-5902.  Fabio Celani. Robust three-axis attitude stabilization for inertial pointing spacecraft using magnetorquers. ScienceDirect: Acta Astronautica, 107:87–96, February–March 2015. doi:10.1016\/j.actaastro.2014.11.027.  E. Carlen and M.C. Carvalho. Linear Algebra: From the Beginning. W. H. Freeman, 2006.  Gemini spacecraft. navigation system. http:\/\/geminiguide.com\/Systems\/navigation.html\/Attitude_Control_and_Maneuver_Electronics. Retrieved September 26, 2021.  Spacecraft overview. new horizons. https:\/\/spaceflight101.com\/newhorizons\/spacecraft-overview\/. Retrieved September 26, 2021.  Sysml activity diagram — enterprise architect user guide. https:\/\/sparxsystems.com\/enterprise_architect_user_guide\/15.2\/model_domains\/sysml_activity_diagram.html. Accessed Nov. 28, 2021.  Uml - activity diagrams. https:\/\/www.tutorialspoint.com\/uml\/uml_activity_diagram.htm. Accessed Nov. 28, 2021.  Carlos Montalvo. Radio controlled aircraft design. accessed:12\/30\/2020 https:\/\/www.youtube.com\/watch?v=RjC3Rou0wkA&list=PL_D7_GvGz-v3d8gRDXWjQaVYfvNOPVx97.  Caughey D.A. Introduction to Aircraft Stability and Control Course Notes for M&AE 5070. , edition, 2011.  Montalvo C. Getting lift curve slopes and drag polars using xflr5. accessed 12\/30\/2020 https:\/\/www.youtube.com\/watch?v=Yz7rmuhGYW0&list=PL_D7_GvGz-v3d8gRDXWjQaVYfvNOPVx97&index=4.  Andre Deperrois. About stability analysis using xflr5 presentation document. June 2008. http:\/\/www.xflr5.tech\/docs\/XFLR5_and_Stability_analysis.pdf.  Xflr5. accessed 12\/30\/2020 http:\/\/www.xflr5.tech\/xflr5.htm.  Ken Meyers. Wing cube loading (wcl). Accessed: 12\/31\/2020 https:\/\/www.sefsd.org\/general-interest\/wing-cube-loading-wcl\/.  Crrcsim. accessed: 12\/30\/2020 https:\/\/sourceforge.net\/projects\/crrcsim\/.  "
}
]
